...
def test_validation_length_mismatch(binary_dataset):
    ...
    with pytest.raises(ValueError, match="same length"):
        ProbaViz(train_data=data, train_target=target[:-1], features=[0, 1])


def test_validation_feature_count(binary_dataset):
    data, target = binary_dataset
    with pytest.raises(ValueError, match="two features"):
        ProbaViz(train_data=data, train_target=target, features=[0])
...
def test_validation_non_numeric_feature():
    ...
    with pytest.raises(ValueError, match="not numeric"):
        ProbaViz(train_data=data, train_target=target, features=["x", "y"])


def test_validation_grid_res_type(binary_dataset):
    data, target = binary_dataset
    with pytest.raises(TypeError, match="grid_res"):
        ProbaViz(train_data=data, train_target=target, features=[0, 1], grid_res=(100, "a"))


def test_fit_populates_train_prediction_cache(binary_dataset):
    data, target = binary_dataset
    model = CountingSVC()
    viz = ProbaViz(model=model, train_data=data, train_target=target, features=[0, 1])
...
def test_model_reassignment_invalidates_prediction_cache(binary_dataset):
    data, target = binary_dataset
    viz = ProbaViz(model=CountingSVC(), train_data=data, train_target=target, features=[0, 1])
...
def test_grid_res_change_updates_mesh_shape(binary_dataset):
    data, target = binary_dataset
    viz = ProbaViz(model=CountingSVC(), train_data=data, train_target=target, features=[0, 1], grid_res=(20, 10))
...
def test_plot_without_model_contour_off(binary_dataset):
    data, target = binary_dataset
    viz = ProbaViz(train_data=data, train_target=target, features=[0, 1])
...
def test_plot_with_model_without_predict_proba_raises(binary_dataset):
    data, target = binary_dataset
    viz = ProbaViz(model=LinearSVC(), train_data=data, train_target=target, features=[0, 1])
